...
export async function buildSystemRules({ projectDir }) {
  ...
  return [
    "Stitches 运行时守卫已启用。",
    "请按顺序使用生命周期：clarify -> design -> plan -> apply -> verify -> archive -> done。",
    selectedLine,
    "活跃变更：",
    activeSummary,
    "每个阶段开始前，运行 `stitches validate --before <state> --change <change-id>`。",
    "每个阶段完成后，运行 `stitches validate --after <state> --change <change-id>`，然后运行 `stitches advance <state> --change <change-id>`。",
    "如果有多个变更同时活跃，运行 `stitches select <change-id>` 选择当前变更。",
    "不要在当前阶段允许的写入范围之外写文件。",
    "Stitches 规划文档默认使用简体中文，除非用户明确要求其他语言。",
    "代码注释、测试、用户文案和项目文档沿用项目现有语言，除非用户明确要求其他语言。",
  ].join("\n");
}
