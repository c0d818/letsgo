...
1. 按“一次一题”澄清需求。存在 2 到 4 个有限选项时，主 Agent 必须调用
   `AskUserQuestion` 生成可直接选择的交互界面；在最佳选项后标注“（推荐）”，并
   保留“其他（请说明）”。只有路径、密钥、需求描述等无法合理枚举的自由文本才
   使用普通输入。不要只输出文字选项、“第一题：”或预告后结束。能从项目或上下文
   推断的信息不要问；中断或恢复后从尚未回答的问题继续，不重复代码分析。
...
- 未获得需求摘要和方案确认时不定稿。
- Subagent 不直接询问用户；发现阻塞问题时结构化交回主 Agent，由主 Agent 按有限
  选项或自由文本规则提问。
- 不手动修改 `status.json`。
