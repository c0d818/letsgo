---
name: stitches-design
description: 编排 Stitches design 阶段的设计 writer 和 reviewer
...
2. 派发 `@stitches-design-writer` 编写 `design.md`，必要时更新变更目录下的 `specs/**`。
3. writer 完成后，派发 `@stitches-reviewer` 只读审查 `design.md` 和
...
6. 主 Agent 执行 `stitches validate --after design`，通过后执行 `stitches advance design`。
