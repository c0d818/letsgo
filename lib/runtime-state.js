...
export async function decideAgentStart({
  ...
  if (enforceNamespace && !String(agentType ?? "").trim().startsWith("lg:")) {
    return deny(`LetsGo 插件只能启动命名空间 Agent：请使用 ${component}`);
  }
  ...
  if (component === REVIEWER) {
    ...
    if (Number(reviewer?.attempts ?? 0) >= 2) {
      return deny(`${REVIEWER} 每阶段最多启动 2 次（初审 + 一次复审）；请停止并报告阻塞`);
    }
  }
  ...
  if (component === REVIEWER && expectedWriter) {
    if (state.agents?.[expectedWriter]?.status !== "completed") {
      return deny(`启动 ${REVIEWER} 前必须先完成 ${expectedWriter}`);
    }
  }
...
export async function validateRuntimeBeforeAdvance({ projectDir, changeId, stage }) {
  ...
  if (writer && state.agents?.[writer]?.status !== "completed") {
    errors.push(`阶段 writer 尚未完成：${writer}`);
  }
...
export function agentNameFromToolInput(toolInput = {}) {
  for (const value of [toolInput.subagent_type, toolInput.agent_type, toolInput.name]) {
    if (typeof value === "string" && value.trim()) {
      return value.trim();
    }
  }
...
function validateAgentPrompt({ prompt, stage, component, expectedWriter }) {
  ...
  const required = role === "reviewer"
    ...
    : ['\"status\":\"ready\"', '\"filesChanged\"', '\"evidence\"', '\"risks\"'];
