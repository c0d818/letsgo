...
export async function recoverWriterCheckpointFromArtifacts({
  projectDir,
  changeId,
  stage,
}) {
  ...
  state.agents[writer] = {
    ...
    result: {
      ...
      status: "ready",
      filesChanged: [],
      evidence: ["continue 根据 validate --after 通过的持久化阶段产物恢复 Writer 检查点"],
      risks: ["运行时事件缺失；仍需 reviewer 独立审查，不能直接 advance"],
    },
    validationErrors: [],
  };
  return writeRuntimeState(projectDir, state);
