...
import { buildRecoveryContext } from "./run-summary.js";
...
export async function decideToolUse({ projectDir, toolName = "", toolInput = {} }) {
  ...
  if (active.length === 0 && isSafeLocalGitDelivery(command)) {
  ...
  if (denied.length > 0) {
    return deny(
      `写入被阻止（${context.state} 阶段）：${denied
        ...
        .join(", ")}。不要重复同一操作或改用 Bash/临时脚本绕过；记录问题并停止`
    );
  }
...
function isSafeLocalGitDelivery(command) {
  const trimmed = String(command ?? "").trim();
  if (hasShellControlOperators(trimmed) || /\bgit\s+commit\b.*\b(--amend|--fixup|--squash)\b/.test(trimmed)) {
    return false;
  }
  if (/^git\s+commit(?:\s|$)/.test(trimmed)) {
    return true;
  }
  if (!/^git\s+add(?:\s|$)/.test(trimmed)) {
    return false;
  }
  const args = tokenize(trimmed).slice(2);
  return args.every(
    (arg) =>
      arg === "--" ||
