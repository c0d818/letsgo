...
- Agent 白名单检查前先比较 continue runtime 与 active/status 身份；冲突时显示双方
  change-id、stage 和项目根目录，不再把选错项目或变更伪装成普通阶段拒绝。
- `/lg:continue` 在派发前强制核对结构化的项目、变更和阶段，禁止相信模型口头总结。

## [0.4.29] - 2026-08-14

### 修复

- `recover` 明确返回来自变更 `status.json` 的权威阶段及唯一恢复命令，不再允许模型根据
  对话摘要自行派发 Writer；恢复后统一交给 `/lg:continue <change-id>` 计算断点动作。
- 错误阶段 Agent 的拒绝信息现在显示阶段事实文件和继续命令，避免模型反复相信过期的
  “recovered-stage” 文本。
...
- 将 `letsgo` CLI 作为插件标准 `bin/letsgo` 一并分发，启用插件后由 Claude Code 自动
  加入 Bash PATH，避免插件已更新但全局 CLI 仍报“未知命令：continue”。
- npm bin 与插件 bin 共用同一入口；普通插件用户不再需要单独安装全局 `letsgo` 包。

## [0.4.27] - 2026-08-14

### 修复

- `letsgo status/validate` 等命令附带 `2>&1` 或 `/dev/null`/`NUL` 诊断重定向时仍按
  LetsGo CLI 自动放行，不再误报“未看到文件路径”。
- 仅剥离不会写普通文件的精确重定向形式；`> status.txt` 等真实文件重定向继续要求
  阶段路径审查。
...
- Apply Writer 的 prompt 协议兼容合法 `partial` 检查点，不再错误要求每个单任务调用都
  包含 `status: ready`，消除“初始化失败后再次派发”的重复显示。
- Writer 或 Reviewer 仍处于 `started` 状态时拒绝同角色再次启动，避免真实并发修改和
  重复 token 消耗；上一调用结束并记录 `partial`/`incomplete` 后仍可继续下一任务。
