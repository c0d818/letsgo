...
- Recover 输出绑定 `status.json` 权威阶段并强制交给 `/lg:continue`，防止第三方模型把
  自然语言中的旧阶段当真后直接派发错误 Writer；Guard 拒绝同时给出事实来源与恢复命令。

- CLI 改为插件内置 `bin/letsgo` 并自动进入 Bash PATH，消除插件版本与全局 npm CLI
  版本漂移；`continue` 等子命令随插件原子更新。

- Bash Guard 在命令分类前归一化 `2>&1` 与 null-device 诊断重定向，确保只读 LetsGo
  CLI 不误弹批准；普通文件重定向仍受写入门禁。

- 后台 Agent 缺失 `SubagentStop` 时，下一次派发前从独立 transcript 恢复合法最终结果；
  兼容旧 `started` 状态且不把仍在运行或无协议的输出误判为完成。
