...
审查 apply 阶段时，必须读取 `tdd-evidence.md` 并逐项对照 `tasks.md` 和代码差异：

- 每个行为任务都有独立的 RED -> GREEN -> REFACTOR Cycle。
- RED 是实际运行后因预期缺失行为失败，不是环境或测试错误。
- GREEN 使用最小实现转绿，REFACTOR 后同一测试仍通过。
- 命令、结果和改动顺序相互一致；证据不足或疑似事后补写时不得通过。
- 只有纯文档、注释、模板、仅测试或其他不改变生产行为的修改可豁免，且必须有验证证据。

缺少证据、行为任务与 Cycle 不对应、RED 未失败或豁免不成立，均为阻塞问题。
